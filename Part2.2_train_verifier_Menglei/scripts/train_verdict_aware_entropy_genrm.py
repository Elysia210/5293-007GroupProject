...
This is the recommended entropy-guided training variant. It upweights
high-entropy reasoning tokens and also gives extra weight to final Yes/No
verdict tokens, so the verifier does not lose the required output format.
"""
...
class VerdictAwareEntropyTrainer(Trainer):
    def __init__(self, *args, tokenizer=None, entropy_cfg=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.tokenizer = tokenizer
        self.entropy_cfg = entropy_cfg or {}
        self.verdict_token_ids = set()

        words = self.entropy_cfg.get("verdict_words", ["Yes", "No"])
        for word in words:
            for variant in [word, " " + word, "\n" + word, ": " + word]:
                for tid in tokenizer.encode(variant, add_special_tokens=False):
                    self.verdict_token_ids.add(tid)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        with torch.no_grad():
            ...
            entropy = entropy / torch.log(torch.tensor(float(vocab_size), device=entropy.device))

            alpha = float(self.entropy_cfg.get("alpha", 0.05))
            weights = 1.0 + alpha * entropy

            topk_ratio = float(self.entropy_cfg.get("topk_ratio", 0.2))
            extra = float(self.entropy_cfg.get("high_entropy_extra_weight", 0.5))
            topk_mask = torch.zeros_like(valid, dtype=torch.bool)

            for b in range(valid.shape[0]):
                pos = torch.where(valid[b])[0]
                if len(pos) == 0:
                    continue
                k = max(1, int(len(pos) * topk_ratio))
                selected = pos[torch.topk(entropy[b, pos], k=k).indices]
                topk_mask[b, selected] = True

            weights = weights + extra * topk_mask.float()

            verdict_mask = torch.zeros_like(valid, dtype=torch.bool)
            for tid in self.verdict_token_ids:
                verdict_mask |= shift_labels.eq(tid)

            verdict_weight = float(self.entropy_cfg.get("verdict_weight", 2.0))
            weights = torch.where(verdict_mask, weights * verdict_weight, weights)
            weights = weights * valid.float()

        loss = (ce * weights).sum() / valid.float().sum().clamp_min(1.0)
...
def main():
    ...
    trainer = VerdictAwareEntropyTrainer(
        model=model,
        args=args_train,
    ...
    print(f"Saved Verdict-Aware Entropy GenRM to {out_dir}")
