...
This script uses an entropy-weighted CE loss. High-entropy tokens receive
larger loss weights, but all assistant tokens remain trainable.
"""
...
class EntropyGuidedTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        with torch.no_grad():
            ...
            topk_ratio = float(self.entropy_cfg.get("topk_ratio", 0.0))
            extra = float(self.entropy_cfg.get("high_entropy_extra_weight", 0.0))
            if topk_ratio > 0 and extra > 0:
...
def main():
    ...
    print(f"Saved Entropy-Guided GenRM to {out_dir}")
